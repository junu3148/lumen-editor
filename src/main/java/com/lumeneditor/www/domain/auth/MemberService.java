...
public interface MemberService {
     ...
     * 로그인이 성공적으로 완료되면, 해당 사용자에 대한 JWT 토큰을 생성하고, 이를 {@code JwtToken} 형태로
     * 반환합니다. 로그인이나 토큰 생성 과정에서 문제가 발생한 경우, 적절한 오류 메시지와 함께 응답이 반환될 수 있습니다.
     *
     * @param user 로그인을 시도할 관리자 사용자의 정보를 담고 있는 {@code AdminUser} 객체.
     * @return 생성된 JWT 토큰을 담고 있는 {@code JwtToken} 객체. 성공적으로 토큰이 생성되면,
     * 토큰이 반환되며, 실패한 경우 null이 반환됩니다.
     */

    JwtToken signInAndGenerateJwtToken(User user);
